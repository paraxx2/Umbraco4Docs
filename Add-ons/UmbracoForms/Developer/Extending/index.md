...
### Data Source Types ###
A data source type enables Umbraco Forms to connect to a custom source of data. A datasource can consist of any kind of storage as long as it is possible to return a list of fields Umbraco Forms can map values to. For exemple: a Database data source can return a list of columns Umbraco Forms can send data to, which enables Umbraco Forms to map a form to a data source. A data source type is responsible for connecting. But Umbraco Forms will always store the data in the same database that is being used for Umbraco and will always use the specific UF* tables and record structure. Umbraco Forms can be extended with custom workflow types which can execute custom storage logic is needed.
